		...
		for (auto &s : strs) {
			...
			for (int i = m; i >= numZeroes; i--) {
				for (int j = n; j >= numOnes; j--) {
					memo[i][j] = max(memo[i][j], memo[i - numZeroes][j - numOnes] + 1);//这里我们新增加一个字符串的时候，计算里面的0和1
																					//更新对应的所有最大组合个数
																		//memo[i - numZeroes][j - numOnes] + 1表示新来的这个字符串会将个数增加1
				}
			}
		}
		return memo[m][n];
...
int main(int argc, char **argv)
{
	...
	cout << test.findMaxForm(strs, 5, 3) << endl;
	return 0;
}